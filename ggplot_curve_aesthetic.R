...
library(grid)
#library(lattice)
setwd("C:/Users/User/OneDrive/Bayesian Big Data/BASiCS and Beyond/BASiCS Robustness/HPC BASiCS/2021/Simulation Experiment/Experiment 1/Local running")
data.path.epsilon0<-getwd()
setwd("C:/Users/User/OneDrive/Bayesian Big Data/BASiCS and Beyond/BASiCS Robustness/HPC BASiCS/2021/Simulation Experiment/Experiment 1/Local running_epsilon=0.25")
data.path.epsilon1quarter<-getwd()
setwd("C:/Users/User/OneDrive/Bayesian Big Data/BASiCS and Beyond/BASiCS Robustness/HPC BASiCS/2021/Simulation Experiment/Experiment 1/Local running_epsilon=0.5")
data.path.epsilon2quarter<-getwd()
setwd("C:/Users/User/OneDrive/Bayesian Big Data/BASiCS and Beyond/BASiCS Robustness/HPC BASiCS/2021/Simulation Experiment/Experiment 1/Local running_epsilon=0.75")
data.path.epsilon3quarter<-getwd()
setwd("C:/Users/User/OneDrive/Bayesian Big Data/BASiCS and Beyond/BASiCS Robustness/HPC BASiCS/2021/Simulation Experiment/Experiment 1/Local running_epsilon=1")
data.path.epsilon4quarter<-getwd()
setwd("C:/Users/User/OneDrive/Bayesian Big Data/BASiCS and Beyond/BASiCS Robustness/HPC BASiCS/2021/Simulation Experiment/Experiment 1/Local running_epsilon")
...
p_delta<-ggplot(aes(x=value, group=name,color=as.factor(epsilon),linetype=as.factor(epsilon)), data=i_df_posterior_delta.long)+
  geom_density(size=1,show.legend = F)+
  stat_density(geom="line",position="identity", size = 0) + 
  #coord_cartesian(xlim = c(0.01, 2))+
...
# scale_color_manual(name=expression(epsilon),
 ...
 # guides(color = guide_legend(override.aes = list(shape=21,fill=c("0"="indianred2", "0.25"="Orange", 
  ...
  theme(axis.text.x = element_text(),axis.title.x = element_text(),panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black"))+
  labs(x=expression(delta[i]),color  = expression(epsilon), linetype = expression(epsilon))+
  facet_wrap(~Df,nrow=3,strip.position='right')
...
p_mu<-ggplot(aes(x=value, group=name,color=as.factor(epsilon),linetype=as.factor(epsilon)), data=i_df_posterior_mu.long)+
  geom_density(size=1,show.legend=F)+
  stat_density(geom="line",position="identity", size = 0) + 
 ...
 # scale_color_manual(name=expression(epsilon),
  ...
  theme(axis.text.x = element_text(),axis.title.x = element_text(),panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black"))+
  labs(x=expression(mu[i]),color  = expression(epsilon), linetype = expression(epsilon))+
  facet_wrap(~Df,nrow=3,strip.position='right')
...
s1<-as.matrix(s1)
n_cell<-nrow(nu1)
#pdf("0_cell_curve_stochsticity.pdf",paper="USr",width=17)
